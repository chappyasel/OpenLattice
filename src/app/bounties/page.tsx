...
function BountyStatusBadge({ status }: { status: string }) {
  const config: Record<string, { color: string; label: string }> = {
    ...
    claimed: { color: "bg-amber-500/10 text-amber-400 border-amber-500/20", label: "Claimed" },
  ...
  return (
    <span className={cn("inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold", c.color)}>
      {c.label}
    </span>
  );
}
